...
const page = () => {
  ...
  fetch('https://jsonplaceholder.typicode.com/posts/100', options)
    .then( (res) => res.json())
    .then( (data) => {
      console.log('/content/investors', 'PUT request successful');
      //console.log(data);
    }).catch( (err) => {
      console.log(err);
    });

  console.log('/content/investors executing...');
  ...
  if(header_list.has('x-message-from-middleware')) { //returns true or false.
    ...
    console.log('/content/investors ReadonlyHeaders', message_header);
  } else {
    console.log('/content/investors ReadonlyHeaders', 'header not available.');
  }
  ...
  if(cookies_list.has('next_response_cookie')){ //returns true or false.
    ...
    console.log('/content/investors cookie', cookie);
    let cookie_name = cookie.name;
    console.log('/content/investors cookie_name', cookie_name);
    let cookie_value = cookie.value;
    console.log('/content/investors cookie_value', cookie_value);
  } else {
    console.log('/content/investors cookie', 'Cookie not available or has expired');
  }
